...
To give you a taste of using Funnel Rocket, here are the steps to download, prepare and query a public e-commerce dataset from [Kaggle](https://www.kaggle.com/).
...
1. A logical name
1. The path where the partitioned files are
1. Which columns hold the group ID (user ID) and timestamp: that's `visitorid` and `timestamp` in the example dataset.
...
1. **First, run all conditions** in the `query.conditions` array.
   ...
   1. If there are none, all groups match this step.
   2. If there's more than one condition, the `query.relation` attribute dictates the logical operator between conditions (`and` by default).

2. **Second, any aggregations** defined under `query.aggregations` are run.
   1. These run only over matching groups, but take into account all rows in these groups.

3. **Lastly, an optional funnel** is run, based only on users matching the conditions (or all).
   1. Aggregations can be requested after each step, or only at the end following the last one.
