...
import pytest
import pandas as pd
from fixtures_n_helpers import InprocessInvokerAndWorker, DEFAULT_GROUP_COUNT, CAT_LONG_TOP, new_dataset, temp_filename
from frocket.common.helpers.utils import timestamped_uuid, bytes_to_ndarray
from frocket.common.tasks.registration import DatasetValidationMode, RegisterArgs, REGISTER_DEFAULT_FILENAME_PATTERN, \
...
from frocket.invoker.jobs.registration_job_builder import RegistrationJobBuilder, VALIDATION_MAX_SAMPLES, \
...
def test_local_single_file_discovery():
    with new_dataset(1) as test_ds:
        job = build_registration_job(test_ds.basepath, DatasetValidationMode.SINGLE)
        job.prerun()

        assert job.parts == test_ds.expected_parts
        assert job.sampled_parts == test_ds.expected_part_ids(job.dataset.id)

        job = build_registration_job(test_ds.basepath, DatasetValidationMode.SAMPLE, uniques=True)
        job.prerun()
        assert job.parts == test_ds.expected_parts
        assert job.sampled_parts == test_ds.expected_part_ids(job.dataset.id)

        job = build_registration_job(test_ds.basepath, DatasetValidationMode.FIRST_LAST, uniques=True, pattern="*t")
        job.prerun()
        assert job.parts == test_ds.expected_parts
        assert job.sampled_parts == test_ds.expected_part_ids(job.dataset.id)
        ...
        job = build_registration_job(test_ds.basepath, DatasetValidationMode.FIRST_LAST)
        job.prerun()
        assert job.parts == test_ds.expected_parts
        assert job.sampled_parts == test_ds.expected_part_ids(job.dataset.id)
...
def do_test_multiple_local_files(parts: int):
    with new_dataset(parts) as test_ds:
        ...
        job = build_registration_job(test_ds.basepath, DatasetValidationMode.SAMPLE)
        job.prerun()
        assert sorted(job.parts.filenames) == sorted(test_ds.expected_parts.filenames)
        assert job.parts.total_size == test_ds.expected_parts.total_size
...
def test_local_missing_path():
    job = build_registration_job("blahblahblah", DatasetValidationMode.SINGLE)
    job.prerun()
...
def test_nofiles_failure():
    ...
    invoker_runner.prerun()
    invoker_runner.enqueue_tasks()


def test_local_nondefault_pattern():
    num_parts = 4
    with new_dataset(num_parts, "blah-", ".123") as test_ds:
        with open(f"{test_ds.basepath}/looklook", "w") as f:
            f.write("herehere")

        mode = DatasetValidationMode.SINGLE
        job = build_registration_job(test_ds.basepath, mode)
        job.prerun()
        assert job.sampled_parts is None

        job = build_registration_job(test_ds.basepath, mode=mode, pattern="blahx*")
        job.prerun()
        assert job.sampled_parts is None

        job = build_registration_job(test_ds.basepath, mode=mode, pattern="bl*.123")
        job.prerun()
...
def test_merged_schema():
    ...
    with new_dataset(num_parts) as test_ds:
        ...
        invoker_runner.prerun()
        invoker_runner.enqueue_tasks()
        assert invoker_runner.num_tasks >= 2 and invoker_runner.num_tasks == len(job.sampled_parts)

        task_results = cast(List[RegistrationTaskResult], invoker_runner.run_tasks())
        paths = [tr.part_id.path for tr in task_results]
        assert sorted(paths) == sorted([sp.path for sp in job.sampled_parts])

        final_job_status = invoker_runner.complete()
        job_result = cast(RegistrationJobResult, invoker_runner.build_result())
...
def test_non_unique_ids():
    with new_dataset(2) as test_ds:
        ...
        job = build_registration_job(test_ds.basepath, DatasetValidationMode.FIRST_LAST, uniques=True)
        invoker_runner = InprocessInvokerAndWorker(
            job, RegistrationTaskRequest, RegistrationTaskResult, RegistrationJobResult)
        invoker_runner.prerun()
        assert len(job.sampled_parts) == 2
        invoker_runner.enqueue_tasks()
        invoker_runner.run_tasks()  # This checks that all *task* results have finished ok
        # The job fails (at the complete() phase) but does not crash
        final_job_status = invoker_runner.complete(assert_success=False)
        assert not final_job_status.success
        result = invoker_runner.build_result(assert_success=False)
